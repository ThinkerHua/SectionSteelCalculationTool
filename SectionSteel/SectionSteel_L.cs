...
namespace SectionSteel {
    ...
    /// <see cref="Pattern_Collection.L_1"/>: <inheritdoc cref="Pattern_Collection.L_1"/><br/>
    /// <see cref="Pattern_Collection.L_2"/>: <inheritdoc cref="Pattern_Collection.L_2"/><br/>
    /// </summary>
    /// <remarks>当匹配到 <see cref="Pattern_Collection.L_2"/> 模式时，
    /// 在国标截面特性表格中查找，同一型号名下按第一个进行匹配。
    /// </remarks>
    public class SectionSteel_L : SectionSteelBase {
        ...
        protected override void SetFieldsValue(SectionSteelBase sender, ProfileTextChangingEventArgs e) {
            ...
            try {
                ...
                Match match = Regex.Match(e.NewText, Pattern_Collection.L_1);
                if (match.Success) {
                    ...
                    match = Regex.Match(e.NewText, Pattern_Collection.L_2);
                    if (!match.Success)
                        throw new MismatchedProfileTextException(e.NewText);
