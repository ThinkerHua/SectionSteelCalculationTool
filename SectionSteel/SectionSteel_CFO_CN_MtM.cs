...
namespace SectionSteel {
    ...
    /// <see cref="Pattern_Collection.CFO_CN_MtM_1"/>: <inheritdoc cref="Pattern_Collection.CFO_CN_MtM_1"/><br/>
    /// <see cref="Pattern_Collection.CFO_CN_MtM_2"/>: <inheritdoc cref="Pattern_Collection.CFO_CN_MtM_2"/><br/>
    /// </summary>
    public class SectionSteel_CFO_CN_MtM : SectionSteelBase {
        ...
        protected override void SetFieldsValue(SectionSteelBase sender, ProfileTextChangingEventArgs e) {
            try {
                ...
                Match match = Regex.Match(e.NewText, Pattern_Collection.CFO_CN_MtM_1);
                if (!match.Success)
                    match = Regex.Match(e.NewText, Pattern_Collection.CFO_CN_MtM_2);
                if (!match.Success)
                    throw new MismatchedProfileTextException(e.NewText);
        ...
        public override string GetWeightFormula(FormulaAccuracyEnum accuracy) {
            ...
            case FormulaAccuracyEnum.ROUGHLY:
            case FormulaAccuracyEnum.PRECISELY:
                formula = $"({h}+{b1}*2+{c1}*2-{t}*4)*{t}*{DENSITY}*2";
                break;
            case FormulaAccuracyEnum.GBDATA:
                if (data != null)
